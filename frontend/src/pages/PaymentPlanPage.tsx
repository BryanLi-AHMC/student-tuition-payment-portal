...
const installments = [
  {
    ...
    amount: '$4,612.50',
    status: 'Scheduled',
    statusClass: 'portal-status--scheduled',
  },
  {
    n: 2,
    due: 'Oct 15, 2026',
    amount: '$4,612.50',
    status: 'Upcoming',
    statusClass: 'portal-status--upcoming',
  },
  {
    n: 3,
    due: 'Nov 15, 2026',
    amount: '$4,612.50',
    status: 'Pending',
    statusClass: 'portal-status--pending',
  },
  {
    n: 4,
    due: 'Dec 15, 2026',
    ...
    <PageLayout title="Payment Plan">
      <main className="portal-page">
            ...
            <tbody>
              {installments.map((row) => (
                <tr key={row.n}>
                  ...
                  <td>{row.amount}</td>
                  <td>
                    <span className={`portal-status ${row.statusClass}`}>{row.status}</span>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <nav className="portal-actions portal-actions--spaced" aria-label="Page actions">
          <Link className="portal-btn portal-btn--secondary" to="/">
            Back to Overview
          </Link>
