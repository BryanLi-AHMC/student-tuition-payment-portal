...
/**
 * First 6–8 digits from the in-browser PAN (before Accept.js tokenization).
 * Keep rules aligned with `backend/src/services/cardFundingFromBin.ts`.
 */
export function cardBinPrefixFromPan(panDigits: string): string | null {
  const digits = panDigits.replace(/\D/g, '').slice(0, 8)
...
export function inferCardFundingFromPan(panDigits: string): CardFunding {
  const p = cardBinPrefixFromPan(panDigits)
  if (p == null) return 'unknown'
  return inferCardFundingFromBinPrefix(p)
}
