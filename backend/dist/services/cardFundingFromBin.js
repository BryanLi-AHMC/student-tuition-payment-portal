 ...
 */
export function normalizeCardBinPrefix(raw) {
 ...
 */
export function inferCardFundingFromBinPrefix(prefix) {
    const digits = prefix.replace(/\D/g, "").slice(0, 8);
    if (digits.length < 6)
        return "unknown";
    const b6 = bin6(digits);
    if (isLikelyDebitBin6(b6))
        return "debit";
    // American Express — treated as credit for surcharge display/settlement.
    if (/^3[47]\d{4}/.test(digits))
        return "credit";
    if (/^4\d{5}/.test(digits))
        return "credit";
    if (/^(5[1-5]|2(2(2[1-9]|[3-9]\d)|[3-6]\d{2}|7[01]\d|720))\d{2}/.test(digits)) {
        return "credit";
    }
    if (/^6(?:011|5\d{2})\d{2}/.test(digits))
        return "credit";
    return "unknown";
}
//# sourceMappingURL=cardFundingFromBin.js.map