import { buildAcademicCourseRecordsFromMarks, scheduleRowsFromAcademicCourseRecords, } from "./studentAcademicCourseRecords.js";
export function quarterOrderForTerm(term) {
...
export function buildAccountCurrentTerm(term, year) {
    const t = term.trim();
...
function sumScheduleUnits(rows) {
    let sum = 0;
    let any = false;
    for (const r of rows) {
...
export function deriveAccountRegistration(args) {
    ...
    if (enrollmentSourceCount > 0) {
        return {
            ...
            courseCount: 0,
            totalUnits: null,
            emptyReason: "Course details could not be loaded for your enrollments for this term.",
        };
    }
    return {
 ...
 */
export function scheduleRowsFromLegacyMarks(marks, studentId) {
    if (marks.length === 0)
        return [];
    const records = buildAcademicCourseRecordsFromMarks(studentId.trim(), marks);
    return scheduleRowsFromAcademicCourseRecords(records);
}
//# sourceMappingURL=studentAccountDashboard.js.map